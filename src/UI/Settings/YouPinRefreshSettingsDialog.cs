...
{
    public sealed class YouPinRefreshSettingsDialog : Form
    {
        ...
        public YouPinRefreshSettingsDialog(Settings config, Action onSaved)
        {
            ...
            _intervalInput = new LiteNumberInput(Math.Max(5, _config.YouPinTrendPageRefreshSec).ToString(), "秒", "", 120);
            ...
            save.Click += (_, __) =>
            {
                int val = _intervalInput.ValueInt;
                if (val < 5)
                {
                ...
                _config.YouPinTrendPageRefreshSec = val;
                _config.Save(); // Save to settings.json
                _onSaved();
                DialogResult = DialogResult.OK;
                Close();
            };
            cancel.Click += (_, __) => dialog_Close();
